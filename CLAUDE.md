...
| Skill | File | Purpose |
|-------|------|---------|
| `/init "username"` | `skills/init.md` | Fetch Twitter user info + tweets → build persona profile → save to `./data/<username>/profile.md` and `./data/<username>/tweets/` |
...
- **Profile-first workflow**: `/init` must run before `/twitter` to establish the user's voice. The profile at `./data/<active>/profile.md` contains style rules that `/twitter` references.
